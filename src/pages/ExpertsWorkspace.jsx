...
export default function ExpertsWorkspace({data,onBack}){
  ...
  const runSearch=async()=>{
    ...
    try{
      const response=await searchCorpus(q,{limit:30,maxPerPublication:3,diversifyByPublication:true})
      const matchedIds=new Set((response.results||[]).map(r=>r.publication_id))
      const experts=buildExperts(publications.filter(p=>matchedIds.has(p.publication_id)),response.results||[]).filter(e=>e.matches.length)
      setResults(experts)
  ...
  const exportPayload={query:query.trim(),experts:filtered,generated_at:new Date().toISOString(),method_note:'Les personnes affichées sont recensées dans le champ auteurs_MI de la base. Les domaines correspondent aux publications auxquelles elles sont associées dans le corpus ; ils ne constituent pas une qualification exhaustive de leur expertise.'}
  ...
  return <main className="page qvl-experts-v01">
    ...
    <section className="qvl-experts-search-card">
      ...
      <div className="qvl-experts-actions"><button className="qvl-experts-primary" onClick={runSearch} disabled={loading}>{loading?'Recherche…':'Rechercher dans le corpus'}</button><button className="qvl-experts-secondary" onClick={reset}>Réinitialiser</button><div className="qvl-experts-method"><Icon name="info" size={16}/><span>Un auteur n’est pas présenté comme expert sur un sujet du seul fait de sa présence dans la base. Avec une requête, seuls les auteurs de publications effectivement retrouvées par le moteur de corpus sont affichés.</span></div></div>
    </section>
    ...
    {!filtered.length?<section className="qvl-experts-empty"><div><Icon name="search" size={32}/><h2>Aucun expert ministériel repéré</h2><p>Le corpus ne rattache aucun auteur ministériel aux publications retrouvées pour ce sujet. Cela ne signifie pas qu’aucun expert n’existe en dehors du corpus.</p></div></section>:<div className="qvl-experts-layout">
      ...
      <aside className="qvl-expert-detail">{activeExpert&&<><div className="qvl-expert-detail-head"><div className="qvl-expert-avatar large">{activeExpert.name.split(/\s+/).slice(0,2).map(x=>x[0]).join('').toUpperCase()}</div><div><span>Expert ministériel recensé</span><h2>{activeExpert.name}</h2></div></div><div className="qvl-expert-detail-body">
        ...
        <div className="qvl-expert-note"><Icon name="info" size={16}/><span>La base recense cette personne via le champ <b>auteurs_MI</b>. Les domaines affichés correspondent aux publications associées dans le corpus ; ils ne valent pas qualification exhaustive de son expertise, de sa fonction ou de son parcours.</span></div>
      </div></> }</aside>
    </div>}
  </main>
}
