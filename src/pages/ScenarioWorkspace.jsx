...
export default function ScenarioWorkspace({initialNeed='',onBack}){
  ...
  return <main className="page qvl-t06-page">
    ...
    {step===2&&<section className="qvl-t06-panel">
      ...
      {!!anchor?.materiaux_valides?.length&&<details className="qvl-t06-relevant" open><summary>Matériaux jugés pertinents pour le sujet ({anchor.materiaux_valides.length})</summary><div className="qvl-t06-relevant-list">{anchor.materiaux_valides.map((s,k)=><SourceProof key={`${s.material_id||s.publication_id||k}-${k}`} source={s}/>)}</div></details>}
