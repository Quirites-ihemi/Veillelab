...
function getTreatmentUrl() {
  // T01, T02, T03 et T04 utilisent désormais explicitement Cloud Run.
...
async function pollCloudJob(baseUrl, jobId, treatmentId) {
  const maxAttempts = treatmentId === 'T01' ? 400 : ['T02','T04'].includes(treatmentId) ? 320 : 180
...
export async function generateTreatment({ treatment, need, publications, contents, nodes = [], relations = [] }) {
  const treatmentId = treatment?.traitement_id
  if (!['T01','T02','T03','T04'].includes(treatmentId)) throw new Error('Cette branche Cloud est actuellement disponible pour T01, T02, T03 et T04.')
