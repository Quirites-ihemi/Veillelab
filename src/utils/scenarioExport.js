...
function sourceRef(source){const s=normSource(source);return [s.publication_id,s.locator?`repère ${s.locator}`:'',s.title,s.organisation,s.year,s.provenance?`provenance ${s.provenance}`:''].filter(Boolean).join(' — ')}
